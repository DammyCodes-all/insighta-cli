...
export async function listProfiles(options) {
    ...
    try {
        ...
        const response = await withSpinner(spinner, () => api.get('/api/profiles', { params }));
...
export async function getProfile(id) {
    ...
    try {
        const response = await withSpinner(spinner, () => api.get(`/api/profiles/${id}`));
        if (response.data) {
...
export async function searchProfiles(query, options) {
    ...
    try {
        ...
        const response = await withSpinner(spinner, () => api.get('/api/profiles/search', { params }));
        if (response.data && response.data.data) {
            printTable(response.data.data, tableColumns);
        }
        else {
...
export async function createProfile(name) {
    ...
    try {
        const response = await withSpinner(spinner, () => api.post('/api/profiles', { name }));
        if (response.data) {
            console.log(JSON.stringify(response.data, null, 2));
        }
    }
...
function handleProfileError(error, defaultMessage) {
    if (error.response?.status === 401) {
        console.log(chalk.red('Error: Session expired. Run: insighta login'));
        process.exit(1);
    }
    else if (error.response?.status === 403) {
        console.log(chalk.red("You don't have permission to perform this action"));
        process.exit(1);
    }
