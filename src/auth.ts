...
export async function login(): Promise<void> {
	...
	try {
		const initResponse = await axios.get<{
			authUrl: string;
			state: string;
		}>(`${BACKEND_URL}/auth/github?mode=cli`);
		...
		const authPromise = new Promise<string>((resolve, reject) => {
			const server = http.createServer((req, res) => {
				...
				const code = queryObject.code as string | undefined;
				const receivedState = queryObject.state as string | undefined;
			...
			server.listen(9876, async () => {
				console.log(
					chalk.yellow('Opening your browser for authentication...')
				);
				console.log(
					chalk.dim(`If the browser doesn't open, visit: ${authUrl}`)
				);
				await open(authUrl);
			});
		});
...
export async function whoami(): Promise<void> {
	try {
		...
		if (axiosError.response?.status === 401) {
			console.log(
				chalk.red('Error: Session expired. Run: insighta login')
			);
			...
			console.log(
				chalk.red(
					`Error: ${axiosError.message || 'Failed to get user info'}`
				)
			);
		}
		process.exit(1);
